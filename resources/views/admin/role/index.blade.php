...
<div class="inner-content">
    <div class="report-table-box">
        <div class="heading-row">
            <h3>Role List</h3>
            <div class="d-flex">
                <a href="{{route('admin.role.create')}}" class="btn btn-add btn-sm">
...
<table class="table">
    ...
    <tbody class="align-middle">
        ...
        <tr>
            ...
            <td> {{ $item->name }}</td>
            <td>{{ $roleCounts[$item->id] ?? 0 }}</td>
            <td>
                <a href="{{route('admin.role.status', $item->id)}}"><span class="btn-sm btn-status btn {{$item->status==1?"bg-success":"bg-danger"}}">{{$item->status==1?"Active":"Inactive"}}</span></a>
            </td>
            <td>
                {{-- <button type="button" class="btn btn-view" title="View"><i class="fa-regular fa-eye"></i></button> --}}
                <a href="{{route('admin.role.edit', $item->id)}}" class="btn btn-edit" title="Edit">Edit</a>
