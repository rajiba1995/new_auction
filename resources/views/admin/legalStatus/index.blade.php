...
<table class="table">
    ...
    <tbody class="align-middle">
        ...
        <tr>
            ...
            <td> {{ $item->name }}</td>
            <td>
                <a href="{{route('admin.legalstatus.status', $item->id)}}"><span class="btn-sm btn-status btn {{$item->status==1?"bg-success":"bg-danger"}}">{{$item->status==1?"Active":"Inactive"}}</span></a>
            </td>
            <td>
                {{-- <button type="button" class="btn btn-view" title="View"><i class="fa-regular fa-eye"></i></button> --}}
                <a href="{{route('admin.legalstatus.edit', $item->id)}}" class="btn btn-edit" title="Edit">Edit</a>
