                            ...
                            <div class="tab-content">
                                <div class="tab-content-wrapper">
                                    <div class="top-content-bar">
                                        <h5 class="text-light">My Buyer Package History</h5>
                                    ...
                                    <div class="content-box">
                                        <table class="table">
                                            ...
                                            <tbody class="align-middle">
                                                @forelse ($buyer_package_history as $key =>$item)
                                                @php
                                                    $buyer_Package = App\Models\Package::where('id',$item->package_Id)->first();
                                                @endphp
                                                <tr>
                                                    <td> {{ $key+1 }}</td>
                                                    <td>{{ $buyer_Package->package_name}}</td>
                                                    <td>{{ $item->package_duration}}</td>
                                                    <td>{{ $item->package_credit}}</td>
                                                    <td>{{ number_format($item->package_amount, 2) }}</td>
                                        ...
                                        {{$buyer_package_history->appends($_GET)->links()}}
                                    </div>
                                </div>
                            </div>
                        </div>
